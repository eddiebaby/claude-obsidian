# claude-obsidian v1.6.0

> DragonScale is now a four-mechanism, opt-in memory layer for LLM wiki vaults.

![DragonScale Memory mechanism overview](../../wiki/meta/dragonscale-mechanism-overview.svg)
...
The current plugin version is `1.6.0`. No local git tag exists for `v1.6.0` in this repository snapshot; tag creation and push are user-owned.
...
![Six-test validation flow](../../wiki/meta/dragonscale-6-test-flow.svg)
...
![Boundary-first autoresearch: frontier picked, pages filed](../../wiki/meta/dragonscale-frontier-graph.svg)
