...
2026-09-16 (session 39, cont.): **Completed the von Franz *Dreams* ingest** (started 2026-06-24, was a partial extraction stopping at p.134). Read the rest of "The Dream of Descartes" chapter (pp. 121-197: full symbolic analysis of all three 1619 dreams, endnotes, and the Glossary appendix) — the whole 220-page book is now fully processed. [[Rene-Descartes]] entity page substantially expanded: the melon as Manichaean light-germ/mandala/I-Ching symbol, "Mr. N." as the overlooked shadow-figure, the second dream's fiery sparks read via the alchemical **scintillae** (plural, pre-unified luminosities vs. the single central [[Lumen-Naturae|lumen naturale]]), the third dream's trickster-and-portraits sequence as the multiplication of the personality, and Descartes's **privatio boni** blind spot as the doctrinal wall his own dreams pressed against. Three new concept pages: [[Privatio-Boni]], [[Scintillae]], [[Larvatus-Prodeo]] (his own motto, "masked do I go forward"). Note for [[dashboard]]: this was a study-domain (depth-psychology) task, not income-generating — flagged per the domain-balance standing instruction.

2026-09-16 (session 39): **Two dreams logged AND analyzed: [[2026-09-16-met-basement-frozen-slab]] + [[2026-09-16-tshirt-stall-mosquito-larvae]] — and Scott's own association landed: the slab scene is Edinger's Dream 2.** Verified against the PDF: *Ego and Archetype* Ch. 8, the dying patient's Dream 2 (pp. 202–204) — the woman half-fused to living rock, freed when the fires are **contained**; association = Michelangelo's "Awakening Giant." The chiasmus is the reading: Edinger's dreamer had excess fire to quench; this series has fire **deferred** (the calcinatio finding) — the frozen figure needs warmth, and the **museum director** (the administrative Self in its clearest costume yet) substitutes chisel-notches in the slab's *end*: accession marks, capture-without-fire — the wiki practice drawn from life. Also in panel 1: ex-girlfriend **Aleksandra** (Russian; series first) — invitation deliberated, not sent = the 09-12 admission question in social costume; gift shop holding goods in his name incl. the literal lube errand = the claim/ownership ceiling in shopping-bag form (watch: the transaction completed). Panel 2 is the corrective: seventh water state (**stagnant-and-breeding** — standing water full of mosquito larvae), knocked over jointly with Niky, clean exit — the **couple-moves-together marker met** (series first, minor key). Watch-list update: the converging question now has four faces — dome built right / descent completed / glass door opened / **frozen figure warmed**. Threads + log updated; still to fill: who's frozen (gender), the director's face, Aleksandra's biography, feeling-tones.

2026-09-15 (session 38, cont. 2): **[[longitudinal-study|Longitudinal Study I]] filed — all 44 dreams (06-25 → 09-12) as one process.** Five phases; SDA agency scores per dream (peak: the single 5 at [[2026-07-14-hotel-third-man-expelled|07-14]]; arc-two plateau in the 3-band whose composition = *competence without claim*); seven developmental lines. Core findings: the failure mode has migrated body → attention → relatedness → grip → **claim/ownership** (the current ceiling — a second "5" would be staying/opening/descending-with, not boundary-drawing); the two slow axes are **father-appeasement** (07-22, corrected in-dream 08-23) and **acquisition/storage**; the central unresolved operation is **calcinatio** — intake accelerating (the fish is being eaten) while fire stays deferred (raw, green, unfired) → dreams captured faster than worked; the Self has gone **administrative** (coordinates 12 & 1884 banked, unwalked) and routes the descent through the **ancestral line** (dump road, breath-engine, 1884 stratum); closing tension = the **split feminine at the glass door** — next expected development is admission. Six consolidated assignments (fire/active-imagination first; storage-unit inventory; family-line dating; sign one wall; walk coordinate 12; fix capture cadence). Doubles as the individuation-app timeline-feature prototype + its first regression test. Flag: 07-04-jung-institute-logistics never made the threads table — classify or prune.

2026-09-15 (session 38, cont.): **All 27 backlog dreams reinterpreted with the vault engine method.** The dream-app pipeline is API-blocked (key at $0 credit), so the re-analysis ran in-session per the engine's own recipe: BM25 retrieval over the corpus per dream, engine writeback schema, full 44-dream context. Each page: fresh corpus-cited **Jungian Analysis** (canonical) + the app's reading preserved as **First Reading**. Retrieval found anchors the app lacked: [[Hesperides]] = the guarded garden verbatim, [[Leviathan]] = the raw-fish meal, [[Lumen-Naturae]] = the ceiling-star scintillae, [[Mercurius]] in the cut studio-roots, [[Bacchus]] as the bar's unseen owner. New series-readings: the maintenance-energy trilogy (08-16/17/21 — the same crew prunes the roots, finds the aperture, scrapes the circle); 08-23's "why did you send that?" as the first *caught* appeasement; 09-08's graded-aggregate speech as the arc's thesis (nothing screened out = load-bearing); 09-12 as the converging closer (ego holds the glass door; the herding creature is the adopted Plott hound's inner twin). Ops: retrieve.py cp1252 crash worked around (PYTHONIOENCODING; fix task spawned); NOTE — a future `dreamapp interpret --force` on these pages would wipe both readings (replaces heading→EOF).

2026-09-15 (session 38): **Dream backlog recovered — 27 new dream pages, 2026-07-14 → 09-12, pulled from the Claude app's Dreams project via data export.** The two-month wiki gap wasn't a lapse in the practice: dream logging had moved into the app's Dreams chat project (Depth-Analyst prompt + 13 Jung/von Franz/Neumann docs). Export → parse → dedupe (the app's 07-14 farm-eggs chat retells the 07-11 wiki triple; the hotel dream was logged twice) → 27 pages in wiki/dreams/ with analyses **ported** from the app sessions, plus a full **Second Arc** section in [[dream-threads]] (at-a-glance table, thread developments, new watch list). Headlines: the gun completes its arc into *language* ([[2026-07-31-summit-anima-descent]] — also the arc's big synchronicity: the New Yorker manslaughter article); **Thread 7's constant breaks** at [[2026-09-08-mall-bar-xanax-aggregate]] — first in-dream consumption, the actual addiction substances, answered by the graded-aggregate foundation image ("a foundation holds only when nothing has been screened out"); the anima **climbs under her own power** (07-31) and ends as **a pair outside the glass door** ([[2026-09-12-dog-creature-glass-door]]); authority moves from citation to claim check ([[2026-08-03-burning-man-tag-12]]); the arc's signature stance is **competence without claim** — helper/guide/schematic-holder who leaves the site ([[2026-08-29-portugal-broken-wall-dome]], [[2026-08-24-long-wall-helpers-eye]]). Self-images bloomed all arc: banyan, blue mosaic building, curbed circle, canyon road, dome, 12, 1884. Capture cadence thinned from nightly (July) to every 2–4 nights (mid-Aug on) — Stage 0 of the app design is the weak link. **Pending:** paste the output-block instruction into the app's Dreams project so future dreams land in `.raw/dreams-inbox/` (no more exports). Dream pages are address-less per series convention; counter untouched at 151.

2026-07-13 (session 37): **Ingested the compass backtesting-engine blueprint → 14 new pages (c-000137–c-000150): [[compass2026-backtesting-engine-blueprint]].** First pure quant-infrastructure ingest: 7 concepts ([[Event-Driven-Backtesting]], [[Continuous-Contract-Construction]] with the dual-series discipline, [[Execution-Realism]], [[Futures-Contract-Mechanics]], [[Information-Driven-Bars]], [[Walk-Forward-Analysis]], [[Combinatorial-Purged-Cross-Validation]]) + 6 tool/vendor entities ([[NautilusTrader]], [[Databento]], [[QuantStart]], [[LEAN-QuantConnect]], [[vectorbt]], [[HftBacktest]]). Context this ingest formalizes: the 2026-07-12→13 sessions also built **`trading/scalppulse/`** — a Schwab-API-connected research pipeline (OAuth working, weekly re-auth; 8.5mo SPY/QQQ/IWM 5m + 7mo /MES + 156-name gap scanner) that killed 3 pattern strategies, an ML meta-filter (walk-forward OOS AUC 0.53 decaying to 0.45), and stocks-in-play ORB (best cell breakeven, Feb-2026 regime carried the sample) — all net-of-costs dead, exactly the blueprint's "costs matter more than indicators" thesis and [[mesfin2026-mnq-intraday-falsification]]'s finding. The blueprint = graduation path for that rig (event-driven core → [[Databento]] tick data → CPCV/DSR validation). Open trading decision: small-cap-gapper sprint (needs Polygon key) vs overnight/swing sprint on existing Schwab pipe. Counter 137→151; index 481→495 pages, 66→67 sources. (flock missing in Git Bash — allocator run manually via counter file, single writer.)

2026-07-11 (session 36): **Ingested 3 papers → 19 new pages (c-000118–c-000136): [[vedfelt2020-integration-vs-conflict-dream-theory|Vedfelt 2020]] (dream theory), [[zeng2023-i-ching-jungian-application|Zeng 2023]] (I Ching), + the [[Transcendent Function]] CW8 mini-ingest.** (1) **Vedfelt** — an *integrative* dream theory: ten cross-validated [[Ten Core Qualities of Dreams|core qualities of dreams]] (important, symbolize, personify, trial-runs, online-to-unconscious-intelligence, pattern-recognition, high-level-communication, condensed, wholeness, energy-landscapes) mapped onto a [[Cybernetic Dream Theory|cybernetic]] self-optimizing-network model of the psyche and a [[Fourfold Brain]] (instinctual/emotional/imaginative/cognitive; vmPFC "absolutely necessary for dreaming"). Vedfelt + [[Structural Dream Analysis|Roesler]] are now the vault's two empirical dream-research anchors for the individuation app. (2) **Zeng** (the paper deferred from session 34) — Jung's decades of [[I Ching Divination|I Ching practice]] in three dimensions (physical/psychological/psychical), the [[Eight Heart Methods of the I Ching]], and 3 contemporary clinicians ([[Jean-Kirsch]], [[Mengchao-Li]], [[Dennis-Merritt]]) with their cross-cultural conflations; extends [[Synchronicity]] as a "Synchronicity-II" practice; same Jingchao Zeng as the 2026 astrology paper. (3) **Transcendent Function** — finally gave the 16+-page dangling link its own CW8-grounded concept page, with Latin [[tertium non datur]] + [[audiatur et altera pars]]. **Latin-phrase rule:** [[religio]], [[relegere]], [[tertium non datur]], [[audiatur et altera pars]] all full pages (Vedfelt had no Latin). **Process:** ran Zeng + TF as two partitioned parallel agents (only Zeng could touch shared files — the session-34 anti-race fix); both stalled on turn budget mid-task (Zeng before its 5 entity pages + counter/index/manifest; TF before its 2nd Latin page). Orchestrator finished the stalled work and did one serialized bookkeeping pass — partition held, no index clobbering, failure mode was truncation not races. Counter 118→137; index 462→481 pages, 64→66 sources.

2026-07-11 (session 35): **Dream triple logged + analyzed — first same-night triple in the series (16 dreams): [[2026-07-11-anima-in-exes-house]], [[2026-07-11-farm-eggs-and-keys]], [[2026-07-11-mountain-house-cat-snow]].** The night runs union → employment → dwelling. (1) The series' **first consummated anima union** — she initiates and completes it — but in the **ex's** house, foreign and stuck; the aftermath repeats the grip fault: refused talk with ex + new boyfriend, stalled getaway, and it ends with the ego *surveilling the house from his car* — the surveillance court's method turned on his own past. New Thread-4 marker: not more intensity, but the anima in a house the ego *owns*. (2) A real local farm: a known, benevolent woman has **saved him a dozen eggs** (varied, some ostrich-huge — the current project-potentials, already set aside) and hires him as **courier** in one conversation — offer → keyring → **signed up**, the series' first executed commitment and its first positive employer-figure (feminine). Consulting business in dream-shorthand: deliver existing local value, corner-store scale, entered by showing up. (3) Move-in to a deep-woods mountain house; the cat plays **joyfully in first snow** — the instinct is in the frozen element the ego has circled for weeks (Thread 3) — and a **bird lands on the cat's head**: first spontaneous spirit–instinct coniunctio, outdoors, unmanaged, while the ego over-fires the thermostat indoors and starts stripping. Grip verdict across the triple: executes where the transaction is simple and the counterpart benevolent (vocational); still fumbles the feeling-domain. Two consecutive police-free nights. Associations partially filled same day (session 36): heat line **confirmed self-set** — the over-firing reading stands; the farm woman confirmed real and characterized (staff at the farm down the street, very pleasant — waking staff, dream employer: the psyche promotes her); no day residue recalled for any of the three. Still open: which ex (and the stall-moment affect); the avoided conversation in waking terms; the farm's actual name; naming the dozen eggs; the pajama/suit couple; the bird's species. (Dream session was interrupted before bookkeeping; log/hot/commit completed by session 36.)

2026-07-11 (session 34): **Batch-ingested 5 sources (4 JOAP 2023 papers + 1 practitioner paper), 41 new pages, addresses c-000075–c-000117.** Dispatched 5 parallel wiki-ingest agents; reconciled a genuine multi-writer race afterward (4 of 5 agents' writes to `concepts/_index.md`/`entities/_index.md`/`sources/_index.md` were clobbered by later agents' stale read-modify-write — the per-file lock serializes writes but doesn't prevent a writer from reading a stale copy; only [[JOAP-68-1-Charura-Bushell-Duoethnographic-Colonialism]]'s entries survived in the index files). Manually merged all missing index entries; the actual wiki pages themselves were all intact (only the shared index files raced). Also completed two agents' truncated work (ran out of turns mid-task): [[filius-regius]] and [[lapis-exilis]] (Ajuonuma had only written 2 of its 5 promised Latin-phrase pages), grounded in existing CW13/CW14 material already in the vault. Dropped one dangling reference (*opus contra naturam*) that a source page cited in its frontmatter but never actually quoted in the extracted text — didn't fabricate a page for it. Fixed 3 pre-existing wikilink-format mismatches (space vs. hyphen/underscore) by adding `aliases:` to [[James-Hillman]], [[Deconstructing-Archetype-Theory]], [[Symbol_and_Myth]] rather than rewriting every citing page.
- [[JOAP-68-1-Charura-Bushell-Duoethnographic-Colonialism]] (Charura & Bushell 2023): new [[Colonizer Archetype]] and [[Ethnic Shadow]] constructs via [[Ubuntu]] philosophy + Jung's layered psyche; direct engagement with the Samuels/Kimbles race critique of Jung.
- [[jones2023-dialectics-sign-symbol-archetype]] (Raya Jones 2023): metatheoretical semiotic audit of "archetype" via Peirce and Bakhtin's "utterance" — archetype theory as a century-long contested communicative act, not a settled proposition. Extends [[Archetype]]'s existing contradiction callouts with a third, independent line of critique (alongside Roesler's genetic/anthropological one).
- [[osterhold2023-chasing-the-numinous]] (Osterhold & Fernandes-Osterhold 2023): the Buddhist Hungry Ghost read onto the psychedelic-therapy "gold rush" as a Western meta-cultural complex (empiricism, heroic conviction, deficiency-solution). Uses the *same* [[Cultural Complex]] machinery as Charura-Bushell's colonial reading — two different collective-shadow contents run through one theory.
- [[ajuonuma2023-alchemical-oedipus]] (Ajuonuma 2023): the full Oedipus cycle (Laius's backstory + Oedipus at Colonus) amplified via the alchemical [[Visio Arislei]] king's-renewal parable; Oedipus read as Laius's redeemer and the Jungian Self rather than Freud's pathology. Direct extension of the vault's CW12/13/14 alchemy coverage into mythological (not dream) material — third worked example this batch of amplification applied beyond individual dreams (myth here, Buddhist cosmology in Osterhold, socio-historical pattern in Charura-Bushell).
- [[turchin-active-imagination-lucid-dreaming]] (Turchin 2024): open-label pilot (n≈100) on Active Imagination as a lucid-dreaming substitute; sitter-viewer protocol; mental-screen/visual-screen typology. Major update to [[Active Imagination]] with a `[!contradiction]` callout — Turchin's scripted, goal-formulating protocol departs from Jung/Hannah/von Franz's "discovery not invention" framing.
- **Known gap flagged, not fixed**: "the transcendent function" is referenced by 16+ vault pages (including this batch's Charura-Bushell page) but has no dedicated concept page — CW8's essay (¶131-193) was only TOC-sampled during the original CW8 ingest. Candidate for a future dedicated mini-ingest.
- **Not part of this batch, dropped in mid-run by a concurrent process**: `.raw/JOAP-68-5-Zeng-I-Ching-Jungian-Application.pdf` — untouched, flagged for Scott.

2026-07-10 (session 33): **Ingested [[li2026-llm-psychology-measurement-survey]]** (arXiv 2606.20603, IEEE TCDS) — survey of LLMs as *psychometric instruments* measuring human psychology. 3 new concepts: [[LLM Psychological Assessment Paradigms]] (active conversational / passive natural-language / multimodal fusion — dream-journal analysis is paradigm 2, check-ins are paradigm 1), [[Machine Theory of Mind]] (Sally-Anne 93%, GPT-4 ≈ adult on Strange Stories, BUT benchmarks measure literal not functional ToM and are called "broken"), [[Psychometric Validity for LLMs]] (the scorecard: convergent validity demonstrated r≈0.44, test-retest/discriminant/criterion/cross-cultural all untested; failure modes = temporal instability, reverse-item mishandling, social desirability bias). Verdict: screening/triage only — "cannot substitute for human-administered assessment in high-stakes contexts"; robust pattern is hybrid LLM + validated instrument (= [[Maker-Checker-Pattern]] with a psychometric checker). **App relevance is the point**: this is the validity map for every LLM-reads-Scott's-material feature; converges with [[Orphic Listening]]'s no-verdict rule from the clinical side; [[Structural Dream Analysis]] flagged as the natural validated anchor for dream material; PsychoGAT (survey items as game scenarios, α=0.97) is prior art for association-test onboarding. Addresses c-000071–74 (manual allocation, flock still broken). Numbers extracted via WebFetch HTML passes — re-verify against PDF before citing.

2026-07-10 (session 32): **Dream logged + analyzed: [[2026-07-10-courthouse-river-fishing]]** — under the Chicago courthouse, a giant muddy river in a Gilded-Age marble hall; the immigrant family's bodega robbed, father called (no cops — first police-free law dream); fishing with the grandfather's popper, fish swarm every cast, **nothing bites**. Scott's own reading (active-imagination trouble) confirmed and corrected: the unconscious isn't dry — material swarms — the failure is inherited surface tackle + sight-method in opaque water + the in-dream commentary track ("I remark as much"). Corrective: live bait — start from a current affect with stakes, fish by feel, enter as participant. [[dream-threads]] updated: Thread 3's stage-change marker **substantially met** (liquid + contained + first direct engagement + first fish; qualifiers: opaque, no exchange — watch for a bite or entry); Thread 2 gets a third interface image (rod-and-line: channel deployed, no current yet); Thread 8's failure migration extends body → attention → relatedness → **grip**. Associations filled same day: Chicago = the great American city Scott "missed out on the most" → the river sits in the **unlived life**, entered at immigrant/bodega scale; the popper is a **real heirloom** from the grandfather's tacklebox (beloved tackle, made for clear water — honor it, retire it for mud); the robbery is fuzzy/off-screen → diffuse drift, no thief to name. Still open: which grandfather, and did they fish together.

2026-07-09 (session 31): **Ingested three JOAP "Process of Transformation" papers (17 new pages, 3 substantive updates).** [[frick2025-spiritual-path-transformation]] (Frick — Jungian analyst + Jesuit): transformation as *circular*, not linear; [[Wilfred Bion|Bion's]] [[Faith in O|O]] (unknowable ultimate reality) as the bridge to the Self; [[Discernment of Spirits]] (Ignatian; three failure modes = spiritual bypassing/neglect/inflation); [[Spiritus contra spiritum]] (Jung→AA's Bill Wilson); [[Createdness]] ("we are created, not creators"); [[In stercore invenitur]]. [[roesler2025-process-of-transformation]] → new concept [[Deconstructing Archetype Theory]]: archetype theory is **four separable strands**; the biological + anthropological ones (the "crystal-lattice/universal images" framing on the existing [[Archetype]] page) are **refuted**, only the *process* theory survives as a "clinical hermeneutics, not vulgar Jungianism" — `[!contradiction]` callout added to both [[Archetype]] and the new page (recorded, not overwritten); SDA is his research exemplar. [[zeng-fraikin2026-synchronicity-postjungian-astrology]]: Jung's synchronicity was a *cautious empirical* claim and his astrology experiment *failed*; he dropped the [[Qualitative Time]] model that Greene & Tarnas later revived — added the failed-experiment caution + Hyde's Synchronicity I/II to [[Synchronicity]]. Frick & Roesler are companions from the 2024 Freiburg conference; cross-linked via [[Individuation]]'s new circular-process section. Concurrency incident: a parallel session grabbed c-000050 mid-write, so all 20 of my addresses shifted +1 to c-000051–c-000070 (counter now 71). `individuation-app/docs/design.md` not edited — Roesler's "clinical hermeneutics" warning flagged as app-relevant if Scott makes the link explicit.

2026-07-08 (session 30): **Ingested two more JOAP papers — one is a major update to existing vault claims, not just new pages.** [[roesler2026-jung-dream-theory-empirical-research]]: [[Structural Dream Analysis]] rewritten from a 5-pattern summary to the full 6-pattern/20-sub-pattern typology (new Pattern 6 "Autonomy"), with three hypotheses now empirically confirmed (H1-H3, 150 case reports + named cases including new entity [[Amalia-X]]) and a "confronts reality, not just compensates" refinement to the compensation question. [[qwarnstrom2026-onesidedness-coriolanus-timon]]: two new concepts, [[One-Sidedness]] (hubris as "virtue gone mad") and [[Enantiodromia]] (the flip-to-opposite law, previously unpaged despite being load-bearing across existing CW material), worked through Coriolanus vs. Timon of Athens; enriched the long-neglected `status: skeleton` [[Superior Function]]/[[Inferior Function]] symbol pages with von Franz's clinical mechanics (the "eagle seizing a mouse" capture reflex). Address backfill: 4 existing pages created 2026-06-25 had never been assigned addresses despite being post-rollout — fixed. Both papers are directly relevant to `individuation-app/docs/design.md`'s dream-ego-agency tracking (SDA's new table is the literal scoring rubric) but design.md itself not touched, per standing pattern of not editing app code during wiki ingests unless asked.

2026-07-08 (session 29): **Ingested [[yago2026-hour-that-never-comes-time-that-remains]]** (Journal of Analytical Psychology) — two paired concepts on psychic temporality: [[The Hour That Never Comes]] (suspended desire/trauma-without-an-event; fidelity-postponement/Demeter vs. anxious paralysis/Sisyphus; threshold dream imagery — doors that won't open, stairways to nowhere) and [[The Time That Remains]] (post-collapse survival time, Fisher's "cancelled future," the wanderer-among-ruins; Yago's real extension: time-collapse may be individuation's condition, not only trauma's wound). New method concept [[Orphic Listening]] — sustain suspended time rather than resolve it — is, read after the fact, the Jungian-tradition grounding for the individuation-app v0.2 anti-confirmation-bias rules added the same day (design.md itself not touched). 9 pages created (1 source, 3 concepts, 5 entities: [[Daniel Françoli Yago]], [[Donald Kalsched]], [[Walter Benjamin]], [[Mark Fisher]], [[Byung-Chul Han]]); 10+ secondary citations deliberately left un-entified. Noted but did not edit: 2026-07-07's dream ([[dream-threads]]) reads as a live hour-that-never-comes case.

2026-07-08 (session 28): **[[Workflow Loops Review]] filed** (wiki/questions/, c-000030) — the current setup mapped against [[Claude Code Loop Taxonomy]]. Verdict: knowledge loops industrialized (hooks, locks, verifier agent), money loops almost all turn-based/manual (sole exception: the equity-book scheduled tasks). Five moves, in build order: (1) **do first — /schedule the monthly sector-momentum rebalance** (code exists; turns the backtest into live decision support); (2) verify-backtest SKILL.md so backtests self-verify (walkforward + DSR + benchmarks + kill criteria); (3) /goal-style stop conditions for the next PRD sleeves (gates already deterministic: ≥2 ticks/trade net, PF ≥1.3 OOS); (4) weekly proactive Hilo consulting-prospect routine — business domain has zero artifacts; (5) fix-the-system hygiene: allocate-address.sh flock still broken (third manual allocation today), app dirs (dream-app, sector-momentum, individuation-app) outside all commit discipline.
...
2026-07-07 (session 25): **Sector-ETF momentum backtest BUILT — first strategy in the vault to move from spec to code.** `sector-momentum/` repo (pandas/numpy only, free Yahoo chart API for the 11 SPDRs + SPY + `^IRX` T-bill hurdle; Stooq now PoW-walled, FRED SSL-flaky). Full data→signal→backtest→DSR→walk-forward harness. Result (2000-2026, net 5bps/side): best config **Top-3 + abs-mom overlay + band(5)** = 10.3% CAGR / Sharpe 0.57 / MaxDD −30.3% vs SPY 8.2% / 0.41 / −55.2%. Full-sample it beats SPY on every axis (13.2x vs 8.1x) and **passes the spec kill criterion**. BUT walk-forward (pick on 2000-15, read 2016-26 untouched): OOS Sharpe 0.71 vs SPY 0.75 — SPY edges it in the pure-bull decade. **The entire edge is crash avoidance** (lead built in 2002 + 2008; 2008: 1.50 vs 0.72). Verdict: **defensive equity sleeve, not standalone alpha** — exactly what the momentum lit + "Sharpe>1 = overfit" discipline predicted. The overlay's job is the drawdown cut (−45%→−30%), not return. [[Sector-ETF Momentum Strategy]] page updated with the results table + status now `[x] Backtested`. **Then ran 6 principled improvement experiments** (`experiments.py`, 15y + full-sample robustness): faster momentum, 3/6/9/12 blend, SPY dual-mom regime gate, top-2 concentration, gate+blend. **NEGATIVE RESULT — every variant is "crash-only": all beat SPY full-sample (has 2008), NONE beats SPY over last 15y (no sustained bear).** Regime gate barely fired (SPY 12-mo mom positive almost the whole 2011-26 window). Hard conclusion: **long-only sector rotation cannot beat buy-and-hold in a bull regime via signal tuning — the win mechanism IS crash avoidance.** Don't re-run momentum-tuning on this universe. **Next money-pointed build**: a genuinely uncorrelated edge = the capacity-constrained plays ([[Post-Earnings Announcement Drift Strategy]], [[LLM Filings Alpha Strategy]]); this repo is the reusable harness (data→signal→backtest→DSR→walk-forward) they inherit.

2026-07-07 (session 24 cont. 2): **Second sweep — how to make the bot BETTER (4 pages + PRD upgrades).** [[Research - Strengthening the LucidFlex Bot Plan]]: three upgrades that live *outside* the falsified OHLCV space. (1) [[baltussen2021-hedging-demand-intraday-momentum]] (JFE) gives intraday momentum a peer-reviewed MECHANISM — dealer gamma hedging + leveraged-ETF rebalancing forcing trades into the close; the gamma-exposure conditioning is NOT in OHLCV bars, so it survives Mesfin → new PRD family #14 "gamma-conditioned intraday momentum", the most promising new sleeve. (2) [[busseti-ryu-boyd-2016-risk-constrained-kelly]] → [[Prop-Firm-Eval-Sizing]]: convex-optimal sizing under P(hit MLL)<β, dominates fractional Kelly; replaces the PRD's ad-hoc 5%-of-MLL. (3) [[cont2021-cross-impact-ofi]]: order flow is execution-timing only (decays sub-minute), not a signal. Plus the eval-as-sizing go/no-go: expected eval spend = $130/pass-prob; if > ~$1,000 (2 payouts) don't run even with a real edge. Open: cheapest dealer-gamma proxy; does the last-30-min window survive the 4:15 hard-flat.
